classdef IiwaRoboespas < handle
    %IIWAROBOESPAS Summary of this class goes here
    ...
    properties(Access=private)
        ...
        ReferencePositionLeft = [-0.7084    0.5604    0.2651   -1.3602    0.2868    1.5976    0.1888]; 
        %-1.43034565448761, 1.277533888816833, -2.738382339477539, 1.576292872428894, -1.003308653831482, -0.4687036871910095, -0.3838373422622681]; % %[-2.27228709005705;1.27528126290555;-2.76711361378742;1.61695719633329;-0.970375948889415;-0.409916398365001; -1.24954618283254]; %
        ReferencePositionRight = [-0.7084    0.5604    0.2651   -1.3602    0.2868    1.5976    0.1888]; 
    ...
    methods
        function obj = IiwaRoboespas(connectROS, commandMode)
            %IIWAAPPLICATIONROBOESPAS Construct an instance of this class
            %   Detailed explanation goes here
            if (connectROS == 1)
                init_ros;
                obj.ROSConnected=1;
                switch (commandMode)
                    ...
                    otherwise
                        disp('Possible inputs: iiwa_stack')
                end
                obj.IiwaCommand.SetVelocity(1);
                obj.IiwaCommand.SetAcceleration(0.1);
                obj.IiwaCommand.SetJerk(0.1);
            end    
        end
        ...
        function [IDPatient, Arm] = SendTrialTrajectory(obj, i_trial)
            ...
            if (obj.ROSConnected)
                %TODO: LoadTrajectory
                [~, traj_out] = obj.IiwaCommand.MoveJVelTraj(obj.traj_commanded);
                obj.CaptureSave(traj_out, obj.NameDataCommanded, 'output');
                obj.DataOutputCaptured{i_trial} = traj_out;
            else
                pause(5);
            end
        end
        function [IDPatient, Arm] = SendReferenceTrajectory(obj)
            ...
            if (obj.ROSConnected)
                %TODO: LoadTrajectory
                [~, traj_out] = obj.IiwaCommand.MoveJVelTraj(obj.traj_commanded);
                obj.CaptureSave(traj_out, obj.NameDataCommanded, 'output_reference')
                obj.DataReferenceCaptured = traj_out;
                traj_spline = IiwaTrajectoryGeneration.BoundedSplineTrajectory(traj_out, obj.OutputSmoothing, 100);
                obj.CaptureSave(traj_spline, obj.NameDataCommanded, 'output_reference_spline');
            else
                pause(5);
            end
        end
        function SendInitialPosition(obj)
            if (obj.ROSConnected)
                if (~isempty(obj.traj_commanded.q))
                    obj.IiwaCommand.MoveJ(obj.traj_commanded.q(:,1));
                else
                    ME=MException('IiwaRoboespas:EmptyDataCommanded', 'Data commanded está vacío'); 
                    throw(ME);
                end
            end
        end
        function ChangeCommandedVelocity(obj, velocity)
            obj.VelocityFactor=velocity;
            if (obj.ROSConnected)
                if (~isempty(obj.DataAdjusted))
                    %%TODO: Add to IiwaTrajectoryGeneration
        ...
        function InputList = DataInputList(obj)
            if (obj.ROSConnected)
                %%TODO InputList = obj.Capture.List(obj.InputIdentifier);
            else
        ...
        function SaveDataInput(obj, id_patient, arm)
            ...
            obj.BuildNameData(id_patient, arm);
            if (obj.ROSConnected)
                traj_captured = obj.IiwaCommand.StopCapture();
                %%TODO: DeletePausesInitEnd
                traj_spline = IiwaTrajectoryGeneration.BoundedSplineTrajectory(traj_captured, obj.InputSmoothing, 100);
                obj.CaptureSave(traj_spline, obj.NameDataInput, 'input');
                obj.CaptureSave(traj_captured, obj.NameDataInput, 'input');
            else
                obj.DefaultCapTrajectoryList=[obj.DefaultCapTrajectoryList, {obj.NameDataInput}];
            end
        end
        function CaptureSave(traj, name, folder)
        	folder = [obj.TrajectoriesFolder, '/', name, '/', folder, '/'];
            mkdir(folder);
            save([folder, traj.name, '.mat'], inputname(1));
        end
        function CommandList = DataCommandedList(obj)
            if (obj.ROSConnected)
                %%TODO
            else
                CommandList = obj.DefaultAdjTrajectoryList;
            end
        end
        function [IDPatient, Arm] = LoadCommanded(obj, name_commanded)
            ...
            if (obj.ROSConnected)
                %%TODO obj.DataAdjusted = obj.Capture.Load(obj.NameDataCommanded, 'entrada');
            end
        end
        ...
        %Transform into trajectory data structure
...
%                             hold on;
        ...
        % Plots trajectories repeatibility
