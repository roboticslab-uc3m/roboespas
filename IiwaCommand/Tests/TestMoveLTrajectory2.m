...
q_start = [-0.4303, 0.3465, 0.0048, -1.1787, 0.0617, 1.3611, -0.4379];

IiwaCommand.MoveJ(q_start);
...
qdotdot_max = 1;
nSegments=100;
traj_spline = IiwaTrajectoryGeneration.BoundedSplineTrajectory(traj_theory, smoothing_min, nSegments);
if (max(max(traj_spline.qdotdot)) < qdotdot_max)
    traj_spline=IiwaTrajectoryGeneration.BoundedSplineTrajectory (traj_in, smoothing_min/10, qdotdot_max, nSegments);
end 
%%
close all
