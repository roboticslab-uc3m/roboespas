...
x_goal=IiwaScrewTheory.ForwardKinematics(q_goal);
format long;
...
traj_straight = IiwaTrajectoryGeneration.TrapezoidalVelocityProfileTrajectory(q_ini, x_goal, control_step_size, 1, 'commanded');
...
n=1;
x_ini=IiwaScrewTheory.ForwardKinematics(q_ini);
qdot1=IiwaScrewTheory.IDK_point(q_curr, traj_straight.xdot(1,:));
...
%         end
    ...
    if (id_curr>=(size(traj_straight.x,1)))
        ...
        traj_output.x(n,:) = IiwaScrewTheory.ForwardKinematics(traj_output.q(n,:));
        break;
    end  
    ...
    if (~isempty(traj_straight.xdot))
        ...
        xdot_exp_A = IiwaScrewTheory.screwA2B_A(x_exp, x_next)/control_step_size;
    end
    xdot_exp_S = IiwaScrewTheory.tfscrew_A2S(xdot_exp_A, x_exp);
    xdot_err_A = IiwaScrewTheory.screwA2B_A(x_curr, x_exp)/control_step_size;
    xdot_err_S = IiwaScrewTheory.tfscrew_A2S(xdot_err_A, x_curr);
