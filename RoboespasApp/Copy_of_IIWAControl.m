...
%% PUBLIC METHODS
        ...
        function FreeJoints(obj)
            %ADDED AS
            %IiwaRoboespas.IiwaCommand.GravityCompensationMode('joint');
            if (obj.ROSConnected)
        ...
        function FreeCartesianCoordinate(obj, coord) %X, Y, Z, A, B, C
            %ADDED as
            %IiwaRoboespas.IiwaCommand.GravityCompensationMode('cartesian',
            %coordinates);
            if (obj.ROSConnected)
        ...
        function BlockRobot(obj)
            %ADDED as IiwaRoboespas.IiwaCommand.GravityCompensationMode('stop');
            if (obj.ROSConnected)
                try
...
%% PRIVATE METHODS
    methods(Access = private)
        ...
        function SetMaximumGeneralVelocity(obj)
            %%ADDED as
            %%IiwaRoboespas.IiwaCommand.SetVelocity/SetAcceleration/SetJerk(1)
            if (obj.ROSConnected)
        ...
        function DataSmooth = CaptureSaveAndSmooth(obj, name, folder, velocity, smooth)
            %%Included inside SaveDataInput using
            %%IiwaTrajectoryGeneration.BoundedSplineTrajectory
            if (obj.ROSConnected)
        ...
        function SendJointPosition(obj, JointPosition)
            %%ADDED as IiwaRoboespas.IiwaCommand.MoveJ(q)
            if (obj.ROSConnected)
...
%             ax = zeros(1,8);
    ...
    methods(Static, Access=public) 
        ...
        function DataSmooth = SmoothData(Data, velocity, smoothFactor)
            ...
            %%Added as IiwaTrajGen.BoundedSpline
