...
public:
  ...
  double q_torque_read[7]; //Watch current torque, do not modify
  double control_step_size; //Watch control_step_size, do not modify
  LBRState robot_state;
...
public:
  ...
  {
    LBRState robot_state=robotState();
    ...
    //Sample time
    control_step_size = robot_state.getSampleTime();
    int timestampSec = robot_state.getTimestampSec();
    int timestampNanosec = robot_state.getTimestampNanoSec();
    if (first_time)
    {
