...
switch dataUsed
    ...
    case 'Kinect'
        ...
        % frecuencia leida por la kinect, no est� comprobada su exactitud
    ...
    case 'IIWA_Kinect'
        ...
        switch trayAnalisis
            case 'completa'
                ...
                %   m�s fiable a la hora de determinar cuando empieza el movimiento.
                % KINECT
                ...
                % utilizando la posici�n del endeffector
            ...
            case 'subida'
                ...
                CMarkers.Handle=FKHandle(1:3,:)';
                % Acoto los datos del IIWA basandome en la misma idea pero en este caso
                % utilizando la posici�n del endeffector pero para Z porque
                % todav�a no esta el sist. de coordenadas adaptado a Opensim
                initialValueZ=CMarkers.Handle(1,3);
...
% V_IIWA=FKHandle(1:3,:)'; %Posici�n desde IIWA ya equidistado
...
for i=0:numMuscles-1
    if model.getMuscles.get(i).hasProperty('gain_factor') %Si tiene la propiedad gain_factor significa que es esp�stico
...
%% Creo variable External Forces (FORCES) - OPCI�N DESECHADA
%
% %Guardo todos los datos en un Storage -> ExternalForcesStorage
...
%% Aplico las fuerzas en External Loads (FORCES) - OPCI�N DESECHADA
...
% rraTool.run
% disp('RRA done');
%% TASKS - C�digo de referencia para futuros usos
% NOTA: Este archivo ya se suministra junto con el resto, el c�digo sirve
% para crear autom�ticamente el archivo xml en el caso de querer
% modificarlo.
% %Marker Tasks
%     %Handle
...
%% AJUSTE EST�TICO DE LOS PARAMETROS MUSCULARES
% [model] = f_MuscleModelChanges(CD_model,MODELO);
% EN TRABAJOS FUTUROS ESTA FUNCI�N SE PUEDE UTILIZAR PARA DEFINIR LOS
% VALORES DE LOS PAR�METROS DE UN MODELO THELEN




%% ELIJO ENTRE EL MODELO DE VELOCIDADES O EL DE FUERZAS - NO FUNCIONA
% ESTE C�DIGO �NICAMENTE SE DEJA POR SI ALGUNA INSTRUCCI�N PUDIERA SERVIR
% DE AYUDA, PERO NO ES FUNCIONAL. CORRESPONDE A LA SOLUCI�N 1 DE LA
% MEMORIA DE EDUARDO S�ENZ
...
% %
% %     %Ejecuto solo un an�lisis cinematico
...
% % disp(strcat(' XML del an�lisis -> ',outfile));
%
% %% NUEVA FD
% if ELECCION == 3
%
...
% %               2- states_degrees.mot -> habr�a que pasarlo a radianes pero
% %               ya es un .mot
%
% % OPCI�N 1:
